...
// Default path.
const std::string defaultpath = getCurrentDirectory() + '/';
// Global variables.
int screenWidth = 600;
...
void RenderSceneCB()
{
    ...
    if (pMesh != nullptr) {
        ...
        pMesh->Render(phongShadingShader);
...
void CreateShaderLib()
{
    ...
    if (!fillColorShader->LoadFromFiles(defaultpath + "shaders/fixed_color.vs", defaultpath + "shaders/fixed_color.fs"))
        exit(1);

    phongShadingShader = new PhongShadingDemoShaderProg();
    if (!phongShadingShader->LoadFromFiles(defaultpath + "shaders/phong_shading_demo.vs", defaultpath + "shaders/phong_shading_demo.fs"))
        exit(1);

    skyboxShader = new SkyboxShaderProg();
    if (!skyboxShader->LoadFromFiles(defaultpath + "shaders/skybox.vs", defaultpath + "shaders/skybox.fs"))
        exit(1);
}
...
int main(int argc, char** argv)
{
    ...
    // Initialization.
    SetupRenderState();
    LoadObjects(SingleFileSelector());
    CreateLights();
    CreateCamera();
    CreateSkybox(defaultpath + "textures/photostudio_02_2k.png");
    CreateShaderLib();
